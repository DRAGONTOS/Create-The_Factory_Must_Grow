...
public class MagmaDepositFeature extends Feature<NoneFeatureConfiguration> {
    public MagmaDepositFeature(Codec<NoneFeatureConfiguration> p_65786_) {
        super(p_65786_);
    }
    ...
    public void placeDeposit(BlockPos startingPos, WorldGenLevel level, RandomSource randomsource) {
        BlockPos pos = startingPos;
        setBlock(level, startingPos, TFMGBlocks.MAGMA_DEPOSIT.getDefaultState());


        for (int i = 0; i < randomsource.nextInt(25); i++) {
            pos = pos.above();

            setBlock(level, pos, TFMGFluids.GASOLINE.get().getSource().defaultFluidState().createLegacyBlock());


            Direction direction1 = Direction.getRandom(randomsource);
            if (direction1.getAxis().isHorizontal())
                setBlock(level, pos.relative(direction1), TFMGFluids.GASOLINE.get().getSource().defaultFluidState().createLegacyBlock());
