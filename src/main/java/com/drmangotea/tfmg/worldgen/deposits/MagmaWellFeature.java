...
public class MagmaWellFeature extends Feature<NoneFeatureConfiguration> {
    public MagmaWellFeature(Codec<NoneFeatureConfiguration> pCodec) {
        super(pCodec);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for(int i = 0; i < height;i++){
            ...
            if(i==0) {
                level.setBlock(startingPos, TFMGBlocks.MAGMA_DEPOSIT.getDefaultState(), 2);
                pos = pos.above();
                continue;
            }
...
//            if(randomsource.nextInt(10)==7) {
                ...
                for (BlockPos pos1 : BlockPos.betweenClosed(new BlockPos((int) area.minX, (int) area.minY, (int) area.minZ), new BlockPos((int) area.maxX, (int) area.maxY, (int) area.maxZ))) {
                    ...
                    if(randomsource.nextInt(10)==7){
                        if(level.getFluidState(pos1).is(Fluids.WATER)||level.getBlockState(pos1).is(Tags.Blocks.SANDS)) {
                            level.setBlock(pos1, TFMGFluids.GASOLINE.getSource().getSource(true).createLegacyBlock(), 3);
            ...
            level.setBlock(pos, TFMGFluids.GASOLINE.getSource().getSource(true).createLegacyBlock(), 3);
